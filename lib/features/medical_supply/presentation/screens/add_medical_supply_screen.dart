...
import 'dart:io';
import '../../../admin/data/models/medical_supply_request_model.dart';
...
class AddMedicalSupplyScreen extends StatefulWidget {
  const AddMedicalSupplyScreen({super.key});

  @override
  State<AddMedicalSupplyScreen> createState() => _AddMedicalSupplyScreenState();
}

class _AddMedicalSupplyScreenState extends State<AddMedicalSupplyScreen> {
  ...
  void _submitForm() async {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      final request = MedicalSupplyRequestModel(
        id: DateTime.now().millisecondsSinceEpoch.toString(),
        name: _supplyNameController.text,
      ...
      context.read<AdminCubit>().addMedicalSupplyDirectly(request);
    }
  }
      ...
      body: Container(
        ...
        child: BlocConsumer<AdminCubit, AdminState>(
          listener: (context, state) async {
            if (state is MedicalSupplyAddedSuccessfully) {
              ...
              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('تم إضافة مكان المستلزمات الطبية بنجاح'),
                    backgroundColor: Colors.green,
                  ),
                );
