...
class MedicalSupplyRequestDetailsScreen extends StatelessWidget {
  final MedicalSupplyRequestModel request;

  const MedicalSupplyRequestDetailsScreen({super.key, required this.request});
    ...
    return Scaffold(
      ...
      body: BlocListener<AdminCubit, AdminState>(
        listener: (context, state) {
          ...
          } else if (state is MedicalSupplyRequestSetToPending) {
            Navigator.pop(context);
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(state.message),
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildSectionTitle('معلومات المستلزمات الطبية'),
                    _buildInfoRow(
                      Icons.location_on,
                      'العنوان',
                      request.address,
  ...
  void _showApproveDialog(BuildContext context) {
    showDialog(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Text('هل أنت متأكد من الموافقة على "${request.name}"؟'),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            onPressed: () {
              ...
              context.read<AdminCubit>().approveMedicalSupplyRequest(request.id);
            },
            style: ElevatedButton.styleFrom(
              backgroundColor: Colors.green,
              foregroundColor: Colors.white,
    ...
    showDialog(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Text('هل أنت متأكد من رفض "${request.name}"؟'),
            const SizedBox(height: 16),
            TextField(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              Navigator.pop(dialogContext);
              context.read<AdminCubit>().rejectMedicalSupplyRequest(
                request.id,
  ...
  void _showSetToPendingDialog(BuildContext context) {
    showDialog(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Text(
          'هل أنت متأكد من تغيير حالة "${request.name}" إلى انتظار؟',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(dialogContext),
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.pop(dialogContext);
              context.read<AdminCubit>().setPendingMedicalSupplyRequest(request.id);
            },
            style: ElevatedButton.styleFrom(
              backgroundColor: Colors.orange,
              foregroundColor: Colors.white,
