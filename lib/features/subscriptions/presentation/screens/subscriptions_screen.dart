...
class _SubscriptionsScreenState extends State<SubscriptionsScreen>
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) =>
          SubscriptionCubit(SubscriptionRepository())..loadAllPlaces(),
      child: Builder(
                  ...
                  child: BlocConsumer<SubscriptionCubit, SubscriptionState>(
                    listener: (context, state) {
                      if (state is SettingsUpdated) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
                              '${state.message} (${state.syncedCount} جديد)',
                            ),
                    ...
                    builder: (context, state) {
                        ...
                        return TabBarView(
                          ...
                          children: [
                            PlacesListWidget(
                              ...
                              onTypeSelected: (type) {
                                print(
                                  '🔍 Filter selected: ${type?.arabicName ?? "الكل"}',
                                );
                              ...
                              onPaymentTap: (place) => _showAddPaymentDialog(
                                context,
                                place,
                                state.settings,
                              ),
                              onSyncPlaces: () => context
                                  .read<SubscriptionCubit>()
                                  .syncAllPlaces(),
                            ),
                            SettingsTabWidget(
                      ...
                      if (state is SubscriptionInitial ||
                          state is PaymentRecorded ||
                          state is SettingsUpdated ||
                          state is NotesUpdated) {
                        // Trigger reload
