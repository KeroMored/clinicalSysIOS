  ...
  static bool isPharmacyOpen({
    ...
    try {
      ...
      if (RegExp(r'^\d{2}:\d{2}-\d{2}:\d{2}$').hasMatch(workingHours.trim())) {
        ...
        print(
          'Open: $openHour:$openMinute ($openTime min), Close: $closeHour:$closeMinute ($closeTime min), Current: ${now.hour}:${now.minute} ($currentMinutes min)',
        );
      ...
      for (var line in hoursList) {
        ...
        if (line.contains('-') && line.contains(':')) {
          final dayRangeMatch = RegExp(
            r'(السبت|الأحد|الاثنين|الثلاثاء|الأربعاء|الخميس|الجمعة)\s*-\s*(السبت|الأحد|الاثنين|الثلاثاء|الأربعاء|الخميس|الجمعة)\s*:',
          ).firstMatch(line);
      ...
      final timeMatch = RegExp(
        r'(\d{1,2}):(\d{2})\s*(ص|م)\s*-\s*(\d{1,2}):(\d{2})\s*(ص|م)',
      ).firstMatch(workingHoursLine);
      ...
      print(
        'Parsed times: Open: $openHour:$openMinute $openPeriod, Close: $closeHour:$closeMinute $closePeriod',
      );
      ...
      print(
        'Converted to minutes: Open: $openTime, Close: $closeTime, Current: $currentMinutes',
      );
